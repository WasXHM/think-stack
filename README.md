...
外部解答和我的理解的创建窗口会把草稿自动保存到当前浏览器 Tab 的 `sessionStorage`，仅用于刷新页面时恢复编辑内容。索引包含主题 ID 与记录类型，因此两个入口互不覆盖；刷新页面或重新打开窗口会恢复草稿，成功提交、点击取消、关闭窗口或按 Escape 放弃编辑时自动清除。草稿不会写入服务端，也不会在不同 Tab 之间共享。
